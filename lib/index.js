...
module.exports = {
	init: function(port, smtpOptions) {
		...
		smtpServer.on("dataReady",function(envelope,callback){
			var email = {sender: null, receivers: {}, data: ""}, mp = new MailParser();
			...
			mp.on("end",function (mail_object) {
				...
				//email.body = email.data.replace(/^\.\./mg,'.');
				...
				*/
				...
				// legacy support
				email.headers.To = email.headers.to;
				email.headers.From = email.headers.from;
				email.headers.Xfolded = email.headers.xfolded;
				email.headers.Xfoldedtab = email.headers.xfoldedtab;
				msgs[++msgid] = email;
